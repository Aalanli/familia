...
fn find_path<'a>(ast: &'a ast::Decl, path: &'a ast::Path, pindex: usize) -> PResult<&'a ast::Decl> {
    match &ast.kind {
...
impl<'a> ASTToIR<'a> {
    fn recursively_construct_types(
        &self,
        ty: &ast::Type,
        ir: &mut ir::IR,
    ) -> PResult<ir::TypeKind> {
        ...
        match &expr.kind {
            ...
            ast::ExprKind::Call { path, args } => {
                ...
                let func_id = *self
                    .path_map
                    .get(path)
                    .map(|decl| self.fn_impl_to_id.get(*decl).unwrap())
                    .unwrap_or_else(|| {
